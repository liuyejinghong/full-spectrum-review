...
在做 `full-spectrum-review` 之前，市面上的 review 技能用了个遍。它们有一个共同点：都是工程性的——镜头对准代码层级的 bug：空指针、边界条件、竞态、漏掉的错误处理。这些当然要抓。但一个系统最贵的错误——架构的形状、业务的对错、机制的必要性——不住在代码行里，这支镜头看不见它们。

我最早让 AI 写交易系统，它交出来的是几个大单体文件：能跑，测试全绿，确实能交易。代价几个月后显现——信号计算和交易执行长在同一处，改一处动全身，没法维护。这份代码逐行看都没有错，错的是形状。工程性 review 对它无话可说。

另一类问题方向相反。有些功能按第一性原理推导，一个直白的实现就够了；AI 却给它叠上层层兜底分支和防御性检查，卷成没人看得懂的状态机。没有一层防御对应一个真实存在的故障模式——每一层都在制造新的状态、新的路径、更大的出错面。这一种病不是代码写少了，是代码写多了。

这两类病，加上"代码忠实实现了从第一天就理解错的业务规则"，就是这个 skill 的起点。它们有同一个特征：**代码没有 bug、测试全绿，但它在一道错题上写出了完美答案。**问题住在模块边界、状态所有权、机制的必要性里，只有站到系统整体，才审得出来。我去市面上找能审这一层的 review skill，没有找到，所以写了 `full-spectrum-review`：让一个没参与开发的第三方 Agent，先从第一性原理重建"这个系统到底在解决什么问题"，再跨模块审判工程正确性、业务真相、可靠性与成本，把局部症状收敛为证据验证过的根因，沉淀成可复审的审计资产。

## 三种 review 形态的差别

前两种，就是上文"工程性 review"的两个具体形态：

| | 逐行 diff review | Checklist 拼盘 | full-spectrum-review |
|---|---|---|---|
| 核心问题 | 这次改动有没有 bug | 类别都打勾了吗 | 系统本身有没有做对 |
| 视野 | diff 和邻近文件 | 每个文件各自过 | 整个项目，跨模块跨生命周期 |
| 业务真相 | 默认代码即对 | 默认代码即对 | 先建业务权威地图，代码只是证据之一 |
| 过度复杂 | 看不见，或到处喊 | 逐项检查，不问该不该存在 | 指控前必须先完成反证调查 |
| 产出 | 一串看完即关的评论 | 一张打勾表 | 稳定 ID + 生命周期，复审接着走 |
| 没审到的部分 | 不交待 | 假装全覆盖 | 覆盖台账如实交待 |

这些差别落到规则上，就是下面六个核心判断。
...
报告里的 Coverage Ledger（覆盖台账）逐块交待两个维度：审到什么深度（deep / sampled / none），以及结论状态（COMPLETE / PARTIAL / NOT_COVERED / INSUFFICIENT_EVIDENCE）。一份"全部通过"但覆盖含糊的报告，不如一份"这两块我不敢下结论"的报告可信。全面审计的"全面"，指的是所有重要边界和流程都进入计划并诚实交待覆盖状态，不是每个文件都逐行扫一遍。
...
- **上下文靠方法省，不靠字数省**——重复阅读由结构消灭：每个模块只有一个 primary owner 负责完整 local review，其他单元只能带着明确声明的横切问题（ownership 边界、端到端 invariant、资源路径）复查同一代码，同问题同深度的重复审查不允许；事实包只装"能改变某个单元看法或做法"的事实。不设任何字数预算：入选的事实不因长度被删，packet 里的候选证据一条不少地保留（packet 漏掉的候选随 worker 上下文一起消失），被排除的只有过程叙事；跟进单元必须先说出它要解决的、仍悬而未决的问题。
