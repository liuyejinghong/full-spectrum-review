...
收敛协议适用边界，修复回归输入泄题与校验规则漂移。保留业务权威、反证、完整集合审查、根因去重和稳定台账。

### Changed

- **定级与证据分离**：取消默认关闭、无现场频率证据的自动 P2 封顶；按受支持场景的后果与暴露定级，未知频率明确记录。缺失测试环境只限制依赖该证据的结论。验证按未决问题选择，不因工具可用而强制执行。
- **既有权衡复审**：原事实、代价与后果仍成立时保留 ACCEPTED；新证据、需求或更小充分修复允许重判，复用稳定 ID。拒绝某方案不等于接受所有后果。
- **集合语义**：保留来源、成员数及逐项核验；参数、状态转换和路由使用自身契约，不再强塞配置治理三分类。
- **输出证据**：Coverage Ledger 模板显示集合成员计数；引用以实际源码为准，纠正继承的报告/packet 引用时说明变化。P0/P1 保留原文片段要求，P2 不因源码中存在未采用的陈旧注释而被迫复述它。
- **跨波次覆盖**：旧覆盖绑定旧版本；通过单元与依赖边界的变更分析复用未受影响覆盖，受影响行为重新审查。报告文件名加入短 revision；澄清工作区等于被审仓库时的审计产物写入与发布授权。
- **Deploy Pack 3→4 / Trading Pack 5→6**：把具体重试、回滚、限频和测试架构改为有条件的失败语义问题；移除发版优先级自动加一，保留身份、授权、保护与外部状态一致性。
- **回归输入隔离**：期望结果移到 `evals/expectations/`，夹具 README 只提供业务背景，移除源码答案注释；修正 case-01 为经济字段丢失，case-02 初始状态一致，case-03 提交边界说明，case-05 错误的循环导入，case-07 用有界研究后果而非通用封顶解释 P2。
- **INDEX 校验**：校验脚本直接读取 index schema 的字段约束，错误类型返回诊断；Schema 明确 provenance 字段非空。脚本仍为 stdlib、专用于 INDEX，不宣称通用 JSON Schema 支持。

### Added

- 中英 README 调用示例与完整短报告示例 `references/example-report.md`。
- INDEX 校验的聚焦回归测试 `scripts/test_validate_fsr.py`。
