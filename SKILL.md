...
Read-only binds **modification of the audited implementation**, not evidence gathering. Use the capabilities the harness and user grant to resolve material uncertainty: target tests, benchmarks, reproducible experiments, or external-contract verification. Keep experiments isolated from live systems and business data; use a sandbox outside the audited tree or clean ephemeral artifacts. Do not impose blanket offline or read/grep-only restrictions when stronger evidence is needed and authorized. Choose checks for the concrete failure or uncertainty they resolve, not merely because they can run. Record unavailable evidence and its impact on the conclusions that depend on it.
