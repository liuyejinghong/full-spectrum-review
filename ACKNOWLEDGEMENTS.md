# Acknowledgements

Full-Spectrum Review is an original review workflow assembled from common production review practices and the open Agent Skills ecosystem.

Useful public references and inspirations include:

- Agent Skills open standard — https://agentskills.io/
- Anthropic Claude Code skills and agent-development examples
- OpenAI Codex public code-review skill patterns
- Sentry public security-review skill patterns
- Public domain-driven-design review skills
- Public production-readiness and architecture-review skills

No upstream skill is vendored or required at runtime. This repository is intended to remain model-neutral and usable by any Agent Skills-compatible client.
